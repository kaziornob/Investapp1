...
class _QuestionTemplateState extends State<QuestionTemplate>
    ...
    if (qusType == 'singleanswermcq' || qusType == 'bool') {
      ...
      return Card(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: <Widget>[
            ...options.map((items) => CheckboxListTile(
                  title: Text(
                      '${items["option_value"]}'
                  ),
                  value: items["checked"],
  ...
  checkAnswer()
  {
    ...
    if(widget.questions[_currentIndex].questionType=="singleanswermcq")
    {
      ...
      var findValue = widget.questions[_currentIndex].userAnswer[0]["value"].toLowerCase().trim();
      var temp = widget.questions[_currentIndex].defaultAnswer[0]['value'].toLowerCase().trim().contains(findValue);
      if(temp==true)
      {
        rightAnsStatus = true;
      }
    ...
    else if(widget.questions[_currentIndex].questionType=="multianswermcq")
    {
      var newArray = [];

      for(var i=0 ; i<widget.questions[_currentIndex].defaultAnswer.length;i++)
      {
          var rightValue = widget.questions[_currentIndex].defaultAnswer[i]['value'].toLowerCase().trim();

          for(var j=0 ; j<widget.questions[_currentIndex].userAnswer.length;j++)
          {
            var userValue = widget.questions[_currentIndex].userAnswer[j]['value'].toLowerCase().trim();
      ...
      if(widget.questions[_currentIndex].defaultAnswer.length == newArray.length
          && widget.questions[_currentIndex].userAnswer.length == widget.questions[_currentIndex].defaultAnswer.length)
      {
        rightAnsStatus = true;
      }
      else
    ...
    var returnData = {"wrongAnsStatus":wrongAnsStatus, "rightAnsStatus": rightAnsStatus};
    return returnData;
  }

  Widget getAnswerList(data)
  {
    if(widget.questions[_currentIndex].userAnswer!=null && widget.questions[_currentIndex].userAnswer.length!=0)
      {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        appBar: new AppBar(
          ...
          iconTheme: new IconThemeData(color: AppThemeData.Colors.AppBarMenuIconColor),
        ),
        body: WillPopScope(
            ...
            child: new Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: getS3Videos("${widget.questions[_currentIndex].videoLink}"),
                  ),
                  Divider(),
                  Container(
                    child: Padding(
