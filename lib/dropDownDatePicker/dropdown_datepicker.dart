  ...
  Widget _buildYearDropdownButton() {
    return Expanded(
      child: _buildDropdownButton(
        ...
        onChanged: (final year) => setState(() => widget._currentDate = widget._currentDate.copyWith(year: year)),
      ),
    );
  }
  ...
  Widget _buildMonthDropdownButton() {
    ...
    if (widget._currentDate?.year != null) {
      if (widget.firstDate.year == widget._currentDate?.year) {
        ...
        if (widget._currentDate?.month != null && widget._currentDate.month < widget.firstDate.month) {
      ...
      if (widget.lastDate.year == widget._currentDate?.year) {
        ...
        if (widget._currentDate?.month != null && widget._currentDate.month > widget.lastDate.month) {
          widget._currentDate = widget._currentDate.copyWith(month: maxMonth);
        }
      }
    } else {
      widget._currentDate = widget._currentDate.copyWith(month: widget._currentDate?.month);
    }
    return Expanded(
      child: _buildDropdownButton(
        items: _buildDropdownMenuItemList(
        ...
        onChanged: (final month) => setState(() => widget._currentDate = widget._currentDate.copyWith(month: month)),
      ),
    );
  }
  ...
  Widget _buildDayDropdownButton() {
    ...
    if (widget._currentDate?.month != null) {
      if (widget._currentDate?.year != null) {
        if (widget.firstDate.year == widget._currentDate?.year &&
          ...
          if (widget._currentDate.day != null && widget.firstDate.day >= widget._currentDate?.day) {
            widget._currentDate = widget._currentDate.copyWith(day: minDay);
          }
        }
        if (widget.lastDate.year == widget._currentDate?.year && widget.lastDate.month <= widget._currentDate?.month) {
          maxDay = widget.lastDate.day;
          if (widget._currentDate.day != null && widget.lastDate.day <= widget._currentDate?.day) {
    ...
    return Expanded(
      child: _buildDropdownButton(
        ...
        onChanged: (final day) => setState(() => widget._currentDate = widget._currentDate.copyWith(day: day)),
      ),
    );
  }
