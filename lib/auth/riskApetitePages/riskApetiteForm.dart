...
class _RiskAptFormState extends State<RiskAptForm> {
  ...
  setData() {
    ...
    if (GlobalInstance.riskInfoQusAns != null &&
        GlobalInstance.riskInfoQusAns.length != 0) {
      for (var i = 0; i < GlobalInstance.riskInfoQusAns.length; i++) {
        print(
            "riskInfoQusAns qus_id: ${GlobalInstance.riskInfoQusAns[i]["qus_id"]}");
        print("qus_id: ${widget.optionData[0].qusID}");

        if (GlobalInstance.riskInfoQusAns[i]["qus_id"] ==
            widget.optionData[0].qusID) {
          setState(() {
            exist = true;
            foundIndex = i;
          });
  ...
  _submit() async {
    ...
    Navigator.of(context, rootNavigator: true)
      ...
      CupertinoPageRoute<void>(
        builder: (BuildContext context) => AnnualReturnForm(
            riskAptType: selectedValue, optionData: optionData),
      ),
    )
        .then((onValue) {
      setState(() {
